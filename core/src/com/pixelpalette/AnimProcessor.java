...
import com.pixelpalette.SpecificOptions.ColorSelectOptions;
import com.pixelpalette.SpecificOptions.EraserOptions;
import com.pixelpalette.SpecificOptions.FillOptions;
import com.pixelpalette.SpecificOptions.PenSelectOptions;
import com.pixelpalette.SpecificOptions.PencilOptions;
import com.pixelpalette.SpecificOptions.SquareSelectOptions;
import com.pixelpalette.SpecificOptions.TextOptions;
import com.pixelpalette.SpecificOptions.WandSelectOptions;
import com.pixelpalette.SpecificUndoActions.AddAnimUndoAction;
import com.pixelpalette.SpecificUndoActions.AddFrameUndoAction;
import com.pixelpalette.SpecificUndoActions.RemoveFrameUndoAction;
import com.pixelpalette.SpecificUndoActions.ResizeUndoAction;
...
import static com.pixelpalette.Canvas.toolLen;
import static com.pixelpalette.PixelApp.pixelSize;

public class AnimProcessor {
    private com.pixelpalette.Canvas[] frames;
    ...
    private com.pixelpalette.UndoAction[] undo, redo;
    private int undoSize;
    private boolean lockUndoStream;
    private com.pixelpalette.ToolOptions[] toolOptions;
    private int curTool;
    private float x, y;
    ...
    public AnimProcessor(com.pixelpalette.DrawStage parent) {
        imgW = 64;
        imgH = 64;
        setup(parent);
    }

    private void setup(com.pixelpalette.DrawStage parent) {
        this.parent = parent;

        lockUndoStream = false;
        animRunning = false;
        undoSize = 25;
        undo = new com.pixelpalette.UndoAction[undoSize];
        redo = new com.pixelpalette.UndoAction[undoSize];
    ...
    public void reset(FileHandle handle) {
        ...
        if(notNull) {
            frames[0].dispose();
            frames[0] = new com.pixelpalette.Canvas(handle, parent, this);
            addToolOptions(0);
        }
        updateFrameWrapper();
    }
    ...
    public void resetToolOptions() {
        ...
        toolOptions = new com.pixelpalette.ToolOptions[toolLen];
    ...
    public void addFrame() {
        if(frames == null) {
            frames = new com.pixelpalette.Canvas[1];
            frames[0] = new com.pixelpalette.Canvas(imgW, imgH, parent, this);
            framePos = 0;
    ...
    public void addFrame(int pos) {
        if(frames != null) {
            ...
            com.pixelpalette.Canvas[] stoFrames = new com.pixelpalette.Canvas[len];
            System.arraycopy(frames, 0, stoFrames, 0, len);
            frames = new com.pixelpalette.Canvas[len + 1];
            ...
            frames[pos] = new com.pixelpalette.Canvas(imgW, imgH, parent, this);
            if(len - pos > 0) {
    ...
    public void setFrame(int pos, com.pixelpalette.Canvas frame) {
        frames[pos].dispose();
        frames[pos] = frame;
        updateFrameWrapper();
    }
    ...
    private void addToolOptions(final int frameNum) {
        com.pixelpalette.Canvas canvas = frames[frameNum];
        for(int i = 0; i < toolLen; i++) {
            canvas.tools[i + 1].setOptions(toolOptions[i]);
        }
    }

    private void addToolOptions(final com.pixelpalette.Canvas canvas) {
        for(int i = 0; i < toolLen; i++) {
            canvas.tools[i + 1].setOptions(toolOptions[i]);
        }
    }
    ...
    public void setImgSizes(int width, int height) {
        imgW = width;
        imgH = height;
        for(com.pixelpalette.Canvas canvas : frames) {
    ...
    public int getPosOfFrame(com.pixelpalette.Canvas frame)
    {
        int ret = -1;
        for (int i = 0; i < frames.length && ret == -1; i++)
        {
            com.pixelpalette.Canvas testFrame = frames[i];
    ...
    public com.pixelpalette.Canvas getCurFrame() {
    ...
    public com.pixelpalette.Canvas[] getFrames() {
        return frames;
    }
    ...
    public void addToUndo(com.pixelpalette.UndoAction action) {
        if(!lockUndoStream) {
            int emptyPixmap = -1;
    ...
    public void addToRedo(com.pixelpalette.UndoAction action) {
        if(!lockUndoStream) {
            int emptyPixmap = -1;
            for(int i = 0; i < undoSize && emptyPixmap == -1; i++) {
                if(redo[i] == null) {
    ...
    public void dispose() {
        ...
        for(com.pixelpalette.UndoAction undoPixmap : undo) {
            if(undoPixmap != null) {
                undoPixmap.dispose();
            }
        }
        for(com.pixelpalette.UndoAction redoPixmap : redo) {
            if(redoPixmap != null) {
                redoPixmap.dispose();
            }
        }
    ...
    public class AnimMenu extends com.pixelpalette.SideMenu {
        private int move;
        private float scroll;
        private com.pixelpalette.TextPrinter textPrinter;
        private com.pixelpalette.TextPrinter largeTextPrinter;
        private com.pixelpalette.MenuList copyMenuList;
        private com.pixelpalette.MenuList animOptions;

        public AnimMenu(final com.pixelpalette.DrawStage parent) {
            super(parent);
            move = -1;
            textPrinter = new com.pixelpalette.TextPrinter(true);
            largeTextPrinter = new com.pixelpalette.TextPrinter(false);
            addCaptureListener(new ClickListener() {
                        ...
                        else
                        {
                            ...
                            if (move == -1)
                            {
                                ...
                                else if (y > getHeight() - 26 * pixelSize && y < getHeight() - 10 * pixelSize)
                                {
                                    ...
                                    {
                                        final com.pixelpalette.NewAnimWindow newAnimWindow = new com.pixelpalette.NewAnimWindow(parent, AnimProcessor.this, "New Anim", getCurFrame().getAnim() != null);
                            ...
                            else if (x > 44 * pixelSize && x < 60 * pixelSize && y > getHeight() - 52 * pixelSize && y < getHeight() - 36 * pixelSize)
                            {
                                move = -1;
                            }
                        }
                ...
                public void touchUp(InputEvent event, float x, float y, int pointer, int button)
                {
                    ...
                    {
                        float distFromTop = (com.pixelpalette.PixelApp.height - 53 * pixelSize) - y + scroll;
                        final int selFrame = (int)(distFromTop / (39 * pixelSize));
                        if (y - scroll < com.pixelpalette.PixelApp.height - 24 * pixelSize)
                        {
                            if (move == -1)
                            {
            ...
            copyMenuList = new com.pixelpalette.MenuList(copyNames, copyActions);
            ...
            final Runnable[] animAtions = {
                    ...
                    () -> {
                        final com.pixelpalette.NewAnimWindow newAnimWindow = new com.pixelpalette.NewAnimWindow(parent, AnimProcessor.this, "Rename Anim", false)
                        {
                            @Override
            ...
            animOptions = new com.pixelpalette.MenuList(animNames, animAtions);
        }

        void moveFrame(float y)
        {
            float distFromTop = (com.pixelpalette.PixelApp.height - 53 * pixelSize) - y + scroll;
            ...
            int numFromTop = (int)(distFromTop / (39 * pixelSize));
            int pos = numFromTop + (above ? 0 : 1);
            if (numFromTop > move)
            {
                pos--;
            }
            if (pos != move)
            {
                com.pixelpalette.Canvas[] framesCpy = new com.pixelpalette.Canvas[frames.length];
                if (move < pos)
                {
                    for (int i = 0; i < move; i++)
                    {
                        framesCpy[i] = frames[i];
                    }
                    for (int i = move; i < pos; i++)
                    {
                        framesCpy[i] = frames[i + 1];
                    }
                    framesCpy[pos] = frames[move];
                    for (int i = pos + 1; i < frames.length; i++)
                    {
                        framesCpy[i] = frames[i];
                    }
                }
                else
                {
                    ...
                    {
                        framesCpy[i + 1] = frames[i];
                    }
                    framesCpy[pos] = frames[move];
                    for (int i = move + 1; i < frames.length; i++)
                    {
                        framesCpy[i] = frames[i];
                    }
                }
        ...
        void checkScroll() {
            float maxScroll = 39 * pixelSize * frames.length - (com.pixelpalette.PixelApp.height - 53 * pixelSize);
            if(scroll > maxScroll) {
                scroll = maxScroll;
            }
            if(scroll < 0) {
        ...
        protected void drawMaximized(Batch batch, float parentAlpha) {
            ...
            if(animations.size() == 0) {
                ...
                for (int i = 0; i < frames.length; i++)
                {
                    ...
                    if (i == move)
                    {
                        y = getHeight() - 89 * pixelSize;
                        spr.setRegion(423, 65, 38, 38);
                        spr.setSize(38 * pixelSize, 38 * pixelSize);
                        spr.setPosition(getX() - 41 * pixelSize, y + 1 * pixelSize);
                        spr.draw(batch, parentAlpha);
                        ...
                        if(ratio > 1f) { // The image is wider than it is tall
                            ...
                            spr.setPosition(getX() - 40 * pixelSize, (y + 2 * pixelSize) + (36 - height) / 2f * pixelSize);
                            spr.draw(batch, parentAlpha);
                            ...
                            spr.setSize(draw.getWidth(), draw.getHeight());
                            spr.setPosition(getX() + (-40 + (36 - width) / 2f) * pixelSize, y + 2 * pixelSize);
                            spr.draw(batch, parentAlpha);
                            draw.setPosition(spr.getX(), spr.getY());
                            draw.draw(batch, parentAlpha);
                        }
                    ...
                    else
                    {
                        ...
                        if (move != -1)
                        {
                            spr.setRegion(621, 290, 100, 3);
                            spr.setSize(getWidth(), 3 * pixelSize);
                            spr.setPosition(getX(), y + 37 * pixelSize);
                            spr.draw(batch, parentAlpha);

                            spr.setRegion(621, 290, 100, 3);
                            spr.setSize(getWidth(), -3 * pixelSize);
                            spr.setPosition(getX(), y + 3 * pixelSize);
                            spr.draw(batch, parentAlpha);
                        }
                        ...
                        if(move == -1 && frames[i].getAnim() != null)
                        {
                            if(!frames[i].getAnim().equals(animName))
                            {
                                animName = frames[i].getAnim();
                                animPos = 1;
                            }
                            String num = Integer.toString(animPos);
                            textWidth = largeTextPrinter.getTextWidth(num);
                            largeTextPrinter.drawText(getX() + 4 * pixelSize + (20 * pixelSize - textWidth) / 2f, y + 12 * pixelSize, num, batch);
                            animPos++;
                        }
                    }
                }
        ...
        private float getSectionY(int i) {
            return com.pixelpalette.PixelApp.height - 53 * pixelSize - (39 * pixelSize * (i + 1)) + scroll;
        }
        ...
        public void minimize() {
            ...
            spr.setPosition(com.pixelpalette.PixelApp.width - spr.getWidth(), 87 * pixelSize);
        ...
        public void maximize() {
            super.maximize();
            setSize(100 * pixelSize, com.pixelpalette.PixelApp.height);
