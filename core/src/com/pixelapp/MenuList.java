...
import static com.pixelapp.PixelApp.*;

public class MenuList extends Actor {
    private final TextPrinter printer = new TextPrinter(true);
    ...
    public MenuList(String[] names, Runnable[] actions)
    {
        ...
        boxWidth += 22 * pixelSize;

        spr = new Sprite((Texture)MANAGER.get("GUI.png"));
        setHeight(17 * pixelSize * len);
        setWidth(boxWidth);
        ...
        addCaptureListener(new ClickListener()
        {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (visible)
                {
                    int pos = (int)Math.floor(y / (17 * pixelSize));
    ...
    public void draw(Batch batch, float parentAlpha) {
        if (visible)
        {
            float middleWidth = boxWidth - 22 * pixelSize;
            for (int i = 0; i < buttons.size(); i++)
            {
                ...
                float y = getY() + (17 * pixelSize * i);

                spr.setRegion(394, yPos, 11, 17);
                spr.setSize(11 * pixelSize, 17 * pixelSize);
                spr.setPosition(getX(), y);
                spr.draw(batch, parentAlpha);

                spr.setRegion(397, yPos, 18, 17);
                spr.setSize(middleWidth, 17 * pixelSize);
                spr.setPosition(getX() + 11 * pixelSize, y);
                spr.draw(batch, parentAlpha);

                spr.setRegion(407, yPos, 11, 17);
                spr.setSize(11 * pixelSize, 17 * pixelSize);
                spr.setPosition(getX() + middleWidth + 11 * pixelSize, y);
                spr.draw(batch, parentAlpha);

                float x = getX() + (boxWidth - buttons.get(i).width) / 2.0f;
                printer.drawText(x, y + 3 * pixelSize, buttons.get(i).name, batch);
            }
        }
    }
