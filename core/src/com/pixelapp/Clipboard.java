...
public class Clipboard extends SideMenu {
    ...
    private void checkScroll() {
        float maxScroll = 39 * pixelSize * clips.length - (PixelApp.height - 24 * pixelSize);
        if(scroll > maxScroll) {
            scroll = maxScroll;
        }
        if(scroll < 0) {
    ...
    protected void drawMaximized(Batch batch, float parentAlpha) {
        ...
        for(int i = 0; i < clips.length; i++) {
            Pixmap clip = clips[i];
            float yPos = PixelApp.height - 63 * pixelSize - 39 * pixelSize * i + scroll;
    ...
    public void maximize() {
        super.maximize();
        setBounds(PixelApp.width - 87 * pixelSize, 0, 87 * pixelSize, PixelApp.height);
    }

    @Override
    public void minimize() {
        super.minimize();
        setBounds(PixelApp.width - 16 * pixelSize, 64 * pixelSize, 16 * pixelSize, 23 * pixelSize);
        spr.setRegion(300, 122, 16, 23);
        spr.setPosition(PixelApp.width - 16 * pixelSize, 64 * pixelSize);
