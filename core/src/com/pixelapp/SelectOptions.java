...
public class SelectOptions extends SideMenu {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if(!hidden() && parent.getDrawMask() && !parent.isDrawing()) {
                    if(minimized || !parent.getMaskVisible()) {
                        ...
                        int pos = (int)((PixelApp.height - y) / (18 * pixelSize));
                        switch (pos) {
                            case 0: // Close
                                minimize();
                                break;
    ...
    private void drawMaskInvisible(Batch batch, float parentAlpha) {
        setBounds(PixelApp.width - 16 * pixelSize, 32 * pixelSize, 16 * pixelSize, 32 * pixelSize);
    ...
    protected void drawMinimized(Batch batch, float parentAlpha) {
        setBounds(PixelApp.width - 29 * pixelSize, 32 * pixelSize, 29 * pixelSize, 32 * pixelSize);
    ...
    protected void drawMaximized(Batch batch, float parentAlpha) {
        setBounds(PixelApp.width - 18 * pixelSize, 0, 18 * pixelSize, PixelApp.height);
    ...
    private void eraseArea() {
        ...
        PixmapUndoAction action = new PixmapUndoAction(canvas, layer, canvas.getCurLayer());
    ...
    public void clearSelection() {
        ...
        parent.setSelectionVisible(true);
        parent.getProcessor().clearRedo();
        SelectUndoAction action = new SelectUndoAction(oldMask, parent.getSelectionMask(), parent, parent.getCanvas());
        parent.getProcessor().addToUndo(action);
    }
    ...
    private Pixmap copy(boolean cut) {
        ...
        int highestX = 0;
        int highestY = 0;
        PixmapUndoAction action = new PixmapUndoAction(parent.getCanvas(), layer, parent.getCanvas().getCurLayer());
        layer.setBlending(Pixmap.Blending.None);
