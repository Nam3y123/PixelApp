...
public class TextPrinter extends Sprite {
    ...
    public TextPrinter(boolean smallText) {
        super((Texture)MANAGER.get("GUI.png"));
        if(fontPixmap == null) {
    ...
    private void drawSmallText(float x, float y, String str, Batch batch) {
        ...
        for(int i = 0; i < str.length(); i++) {
            ...
            if(c >= 32) {
                ...
                setPosition(x + ofs * pixelSize, y);
                setSize(size * pixelSize, pixelSize * 9);
                draw(batch);
                ofs += size - 1;
            }
        }
    ...
    private void drawBigText(float x, float y, String str, Batch batch) {
        float drawWidth = pixelSize * 10;
        float drawHeight = pixelSize * 12;
        float ofs = 0;
        for(int i = 0; i < str.length(); i++) {
            char c = str.charAt(i);
            int pos = 0;
    ...
    public float getTextWidth(String str) {
        return getTextPixelWidth(str) * pixelSize;
    }
