...
import com.pixelapp.SpecificUndoActions.AddLayerUndoAction;
import com.pixelapp.SpecificUndoActions.MoveLayerUndoAction;
...
public class LayerSelector extends SideMenu {
    ...
    public LayerSelector(final DrawStage parent) {
        super(parent);
        move = -1;
        layerOptionsOpen = -1;
        ...
        setBounds(PixelApp.width - 105 * pixelSize, 0, 105 * pixelSize, PixelApp.height);
        addCaptureListener(new ClickListener() {
                ...
                if(ret) {
                    if(!parent.isDrawing()) {
                        if(minimized) {
                            ...
                            canScroll = y < PixelApp.height - 24 * pixelSize;
                            scrollChanged = !canScroll;
                            storeY = y;
                            storeScroll = scroll;
                            if(move == -1) {
                                if(y > PixelApp.height - 20 * pixelSize && y < PixelApp.height - 4 * pixelSize) {
                                ...
                                if(x > 71 * pixelSize && y > PixelApp.height - 17 * pixelSize) {
                                    minimize();
                                }
                            } else if(y > PixelApp.height - 20 * pixelSize && y < PixelApp.height - 4 * pixelSize && x > 44 * pixelSize && x < 60 * pixelSize) {
                                move = -1;
                            }
                        }
                    }
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer) {
                if(canScroll) {
                    scroll += y - storeY;
                    storeY = y;
                    float maxScroll = 39 * pixelSize * parent.getCanvas().layers.size() - (PixelApp.height - 24 * pixelSize);
                    if(scroll > maxScroll) {
                        scroll = maxScroll;
                    }
                    if(scroll < 0) {
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if(!scrollChanged && canScroll) {
                    if(move == -1) {
                        float distFromTop = (PixelApp.height - 24 * pixelSize) - y + scroll;
                        int numFromTop = (int)(distFromTop / (39 * pixelSize)) + 1;
                        Canvas canvas = parent.getCanvas();
                        int newLayer = canvas.layers.size() - numFromTop;
                        if(y - scroll < PixelApp.height - 24 * pixelSize && x > 22 * pixelSize && x < 58 * pixelSize) {
                    ...
                    } else if(y - scroll < PixelApp.height - 24 * pixelSize) {
                        moveLayer(y - scroll);
                    }
                }
            }
    ...
    private void moveLayer(float y) {
        float distFromTop = (PixelApp.height - 24 * pixelSize) - y;
        ...
        if(secondLayer > move) {
            canvas.setCurLayer(secondLayer + (above ? 0 : -1));
            canvas.layers.add(secondLayer + (above ? 0 : -1), temp);
        } else {
            canvas.setCurLayer(secondLayer + (above ? 1 : 0));
            canvas.layers.add(secondLayer + (above ? 1 : 0), temp);
        }
        parent.getProcessor().addToUndo(new MoveLayerUndoAction(canvas, canvas.getCurLayer(), move));
        parent.getProcessor().clearRedo();
        move = -1;
    }

    protected void drawMinimized(Batch batch, float parentAlpha) {
        setBounds(PixelApp.width - 16 * pixelSize, 0, 16 * pixelSize, 32 * pixelSize);
    ...
    protected void drawMaximized(Batch batch, float parentAlpha) {
        setBounds(PixelApp.width - 87 * pixelSize, 0, 105 * pixelSize, PixelApp.height);
        spr.setRegion(317, 24, 87, 3);
        spr.setSize(87 * pixelSize, PixelApp.height - 24 * pixelSize);
        spr.setPosition(getX(), 0);
        spr.draw(batch, parentAlpha);
        ...
        for(int i = 0; i < layersDraw.length; i++) {
            ...
            float y = PixelApp.height - 24 * pixelSize - (spr.getHeight() * (i + 1)) + scroll;
        ...
        spr.setPosition(getX(), PixelApp.height - spr.getHeight());
        spr.draw(batch, parentAlpha);
    }
